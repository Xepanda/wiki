...
- id: 'xiaoai_turn_off_haier_ac'
  alias: '小爱音箱语音联动：关闭海尔空调'
  description: '当小爱音箱触发 关闭空调 时，通过涂鸦万能遥控器发射红外码'
  trigger:
    - platform: state
      entity_id: sensor.xiaomi_l05b_b558_conversation
    - platform: event
  ...
  condition:
    - condition: template
      value_template: >-
        {{
          ('关闭空调' in (trigger.to_state.state | default(''))) or
          ('关空调' in (trigger.to_state.state | default(''))) or
          ('关闭空调' in (trigger.event.data.query | default('')))
        }}
  action:
    - service: scene.turn_on
      target:
        entity_id: scene.guan_bi_kong_diao
    - service: text.set_value
      target:
        entity_id: text.xiaomi_l05b_b558_play_text
      data:
